<template>
  ...
  const diaryData = JSON.parse(router.currentRoute.value.params.diaryData);
  ...
  const updateDiary = async () => {
    try {
      ...
      const response = await axios.put(`http://localhost:8000/api/diaries/update/${diaryData.id}`, {
